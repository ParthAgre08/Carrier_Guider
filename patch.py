...
file_path = "c:\\Users\\btm9d\\Desktop\\Techathon\\Carrier_Guider\\app.py"
with open(file_path, "r", encoding='utf-8') as f:
    content = f.read()

# I will find the starting line for `@app.route("/assessment"` and replace everything from there to the end.
search_str = '@app.route("/assessment" , methods =["GET","POST"])'
idx = content.find(search_str)

new_content = content[:idx] + '''@app.route("/assessment", methods=["POST"])
def assessment():
